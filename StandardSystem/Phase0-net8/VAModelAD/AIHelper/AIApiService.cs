...
namespace VAModelAD.AIHelper
{
    public class AIApiService : IDisposable
    {
        ...
        public static string InitAIEndPoint(string url)
        {
            ...
            {
                //tokenKey = EvaluationCheck.GetTokenKey();
                // net8/headless: read appSettings directly (no web.config OpenWebConfiguration);
                // the host supplies AIEndPoint via appsettings/app.config or environment config.
                _aiEndpoint = System.Configuration.ConfigurationManager.AppSettings["AIEndPoint"] ?? "";
            }
            catch (Exception ex)
            {
                _aiEndpoint = "";
