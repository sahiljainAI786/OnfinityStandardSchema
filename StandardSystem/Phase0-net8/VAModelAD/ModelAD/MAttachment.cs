...
namespace VAdvantage.Model
{
    ...
    public class MAttachment : X_AD_Attachment
    {
        ...
        private bool SaveLOBData()
        {
            ...
            // net8: standard ZIP (DEFLATE) via System.IO.Compression — format-compatible with the
            // previous java.util.zip output, so existing stored attachment blobs still round-trip.
            System.IO.Compression.ZipArchive zip =
                new System.IO.Compression.ZipArchive(bOut, System.IO.Compression.ZipArchiveMode.Create, true);
            //zip.sete
            //
            try
            {
                ...
                for (int i = 0; i < isize; i++)
                {
                    ...
                    // make zip entry + write its data
                    var entry = zip.CreateEntry(item.GetName(), System.IO.Compression.CompressionLevel.Optimal);
                    byte[] data = item.GetData();
                    using (var entryStream = entry.Open())
                        entryStream.Write(data, 0, data.Length);
                }
                // finalize the archive (writes the central directory) before reading the bytes
                zip.Dispose();

                byte[] sObjData = bOut.ToArray();
                bOut.Dispose();
        ...
        private bool LoadLOBData()
        {
            ...
            try
            {
                ...
                using (var inBt = new System.IO.MemoryStream(sdata))
                using (var zip = new System.IO.Compression.ZipArchive(inBt, System.IO.Compression.ZipArchiveMode.Read))
                {
                    ...
                    foreach (var entry in zip.Entries)
                    {
                        ...
                        // get file name (FullName = the name as stored, matches java getName())
                        string name = entry.FullName;
                        itm.Name = name;

                        byte[] sdataEntry;
                        using (var outBt = new System.IO.MemoryStream())
                        using (var entryStream = entry.Open())
                        {
                            entryStream.CopyTo(outBt);
                            sdataEntry = outBt.ToArray();
                        }
                        byte[] dataEntry = ConvertToByte(sdataEntry);

                        log.Fine(name + " - size=" + dataEntry.Length
                            + " - zip=" + entry.CompressedLength + "(" + entry.Length + ")");
        ...
        {
            string serverLocation = System.Configuration.ConfigurationManager.AppSettings["serverLocation"];
        ...
        public string GetFile(int AD_AttachmentLine_ID)
        {
            try
            {
                ...
                if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    //unzipfile
                    ICSharpCode.SharpZipLib.Zip.FastZip z = new ICSharpCode.SharpZipLib.Zip.FastZip();
                    ICSharpCode.SharpZipLib.Zip.ZipStrings.CodePage = 720;
