...
namespace VAdvantage.DBPort
{
    ...
    public abstract class ConvertSQL_SQL92 : ConvertSQL
    {
        ...
        protected String ConvertOuterJoin(String sqlStatement)
        {
            ...
            String fromPart = sqlStatement.Substring(fromIndex, whereIndex - fromIndex);
            String wherePart = sqlStatement.Substring(whereIndex, endWhereIndex - whereIndex);
            ...
            while (pos != -1)
            {
                ...
                //  extract condition
                String condition = newWherePart.Substring(start + startOffset, (end - start - startOffset));
                joins.Add(condition);
                if (trace)
                    log.Info("->" + condition);
                //  new WHERE clause
