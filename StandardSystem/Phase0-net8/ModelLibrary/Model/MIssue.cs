...
namespace VAdvantage.Model
{
   ...
   public static MIssue Create(LogRecord record)
	...
	public static MIssue Create(Ctx ctx, String hexInput)
	{
		...
		{
			// net8: the hex payload was Java-serialized (java.io.ObjectInputStream). Java
			// serialization is not portable to .NET, so this legacy error-report import path is
			// not supported on the headless engine.
			_log.Log(Level.WARNING, "MIssue.Create from Java-serialized hex not supported on net8");
			return null;
