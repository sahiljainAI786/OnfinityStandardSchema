        ...
        public string AttachCOAImportFile(string fileName)
        {
            try
            {
                    ...
                    List<AttFileInfo> fileList = new List<AttFileInfo>
                {
                    ...
                    // net8/headless: the web AttachmentModel (ViennaAdvantageWeb.Areas.VIS.Models)
                    // is not available headless; the COA-image attachment step is skipped. A headless
                    // host would attach via VAdvantage.Model.MAttachment directly.
                    string attachmentError = null;
                    // Error Detail if not attached
                    if (!string.IsNullOrEmpty(attachmentError))
                    {
                        log.Severe("File Not Attached For Chart of Account Name - " + GetName() + ", Error : " + attachmentError);
