...
namespace VAdvantage.Model
{
    ...
    /// <summary>
    /// Model Actions
    ///base class used for override modelDocactions
    ///dependecy injected by ModelValidator class
    /// </summary>
    public abstract class ModelActions
    {
        ...
        /// Prepare It
        /// </summary>
        /// <param name="skipBase">skipped base if true</param>
        /// <returns></returns>
    ...
    /// get all client classes and regiter them at first run
    /// </summary>
            ...
            StringBuilder sqlFrom, StringBuilder sqlOrder)
        {
            ...
            for (int i = 0; i < _validators.Count; i++)
            {
                ...
                if (validator.GetAD_Client_ID() == 0 || validator.GetAD_Client_ID() == AD_Client_ID)
                {
                    try
                    {
                        bool bb = true;// validator.UpdateInfoColumns(columns, sqlFrom, sqlOrder);
                        if (bb)
                            retValue = true;
                    }
                    catch (Exception e)
