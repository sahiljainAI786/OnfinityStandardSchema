...
@return Supervisor for this user/organization - used for escalation and approval */
        ...
        /** Set Escalate to Approver Supervisor.
        @param EsclAppSupervisor Escalation to be sent to approver's supervisor */
        public void SetEsclAppSupervisor(Boolean EsclAppSupervisor)
        {
            Set_Value("EsclAppSupervisor", EsclAppSupervisor);
        }

        /** Get Escalate to Approver Supervisor.
        @return Escalation to be sent to approver's supervisor */
        public Boolean IsEsclAppSupervisor()
        {
            Object oo = Get_Value("EsclAppSupervisor");
            if (oo != null)
            {
        ...
        /** Set Escalation With Forward.
        @param EsclForward Escalation to be done with forwarding the doument */
        public void SetEsclForward(Boolean EsclForward)
        {
            Set_Value("EsclForward", EsclForward);
        }

        /** Get Escalation With Forward.
        @return Escalation to be done with forwarding the doument */
        public Boolean IsEsclForward()
        {
            Object oo = Get_Value("EsclForward");
            if (oo != null)
            {
                if (oo.GetType() == typeof(bool))
                    return Convert.ToBoolean(oo);
        ...
        /** Set Escalate on Reminder.
        @param EsclReminder Escalation to be sent once reached number of reminders */
        public void SetEsclReminder(int EsclReminder)
        {
            Set_Value("EsclReminder", EsclReminder);

        }

        /** Get Escalate on Reminder.
        @return Escalation to be sent once reached number of reminders */
        public int GetEsclReminder()
        {
            Object ii = Get_Value("EsclReminder");
            if (ii == null)
                return 0;
            return Convert.ToInt32(ii);
        }

        /** Set Workflow Incharge.
