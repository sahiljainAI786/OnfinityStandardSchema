...
.panel-body {
  ...
  --panel-anchor: clamp(16px, 1vw, 18px);         /* single source of truth */
...
- Rendered range: `16–18px`.
  - `≤1600px` viewport → `16px` (floor)
  - `1600–1800px` viewport → linear ramp `16→18px`
  - `≥1800px` viewport → `18px` (ceiling)
...
- Any panel implementation setting the anchor to a value other than `clamp(16px, 1vw, 18px)` (or the equivalent `var(--panel-anchor)`): broken — the whole point of the single formula is that right and bottom panels compute the same size on the same window. A one-off coefficient defeats it.
...
Bottom panels use the **canonical panel anchor** defined once in `Panel Foundation > Rule 3`: `clamp(16px, 1vw, 18px)`, applied via the shared three-line block (`container-type: inline-size` + `--panel-anchor` + `font-size: var(--panel-anchor)`). No bottom-panel-specific coefficient — the whole point of the single-formula rule is that bottom and right panels compute the same size on the same window.
...
- Rule of thumb: rendered anchor `16–18px` on any panel (bottom, right, header, detail pane); `16–20px` on dashboard widgets. Grid body cells at `0.75em` therefore land at `~12–13.5px` on panels and `~12–15px` on widgets — the shared token set (`0.75em` grid body cell, `0.6875em` grid header, `0.875em` row title, `1em` section title, three-tone divider hierarchy — see `dashboard-widgets.md > Grid Data Widget`) is what keeps sizes consistent across surfaces; the anchor is what maps those tokens to physical pixels.
...
Right panels use the **canonical panel anchor** defined once in `Panel Foundation > Rule 3`: `clamp(16px, 1vw, 18px)`, applied via the shared three-line block (`container-type: inline-size` + `--panel-anchor` + `font-size: var(--panel-anchor)`). No right-panel-specific coefficient — a right panel next to a bottom panel on the same window renders at the same physical size *by construction*.
...
All right-panel **body** content uses `Roboto` and is sized in `em` against the canonical panel-body anchor (`clamp(16px, 1vw, 18px)` — see `Panel Foundation > Rule 3`). The panel **header** is chrome — its `em` values resolve against the document baseline (`1em = 16px`), unaffected by the body anchor.
