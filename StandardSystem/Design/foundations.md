...
- **Right-panel and bottom-panel body typography and scalable spacing → `em` against the canonical panel-body anchor** (`clamp(16px, 1vw, 18px)`). One formula for every panel type; a right panel next to a bottom panel on the same window renders at the same physical size. See [windows-and-panels.md](./windows-and-panels.md) → `Panel Foundation > Rule 3`.
