...
namespace VAdvantage.Model
{
    public class MTree : X_AD_Tree
    {
        ...
        private void TrimTree(VTreeNode tn)
        {
            ...
            while (en.MoveNext())
            {
                ...
                else
                {
                    ...
                    if (node.Level == 1 && node.IsSummary)
                    {
                        if (tn == node)
                            continue;
                        VTreeNodeCollection nodeDetails = node.Nodes;
        ...
        /// Seach for tree node
        /// </summary>
        /// <param name="tNodes">VTreeNode Collection</param>
        /// <param name="strFind">node name</param>
        /// <returns>VTreeNode</returns>
        private VTreeNode FindTreeNode(VTreeNodeCollection tNodes, string strFind)
        {
            foreach (VTreeNode tn in tNodes)
            {
                if (tn.Name == strFind)
                {
                    return (VTreeNode)tn;
                ...
                {
                    //Call recursive
                    VTreeNode match = FindTreeNode(tn.Nodes, strFind);
                    if (match != null)
                    {
