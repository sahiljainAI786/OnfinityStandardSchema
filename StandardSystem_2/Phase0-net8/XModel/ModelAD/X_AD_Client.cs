...
@return Allow Non Item on Ship/Receipt */
        ...
        /** Set Round off Line Tax Amount.
        @param IsRoundLineTaxAmt Round off Line Tax Amount */
        public void SetIsRoundLineTaxAmt(Boolean IsRoundLineTaxAmt)
        {
            Set_Value("IsRoundLineTaxAmt", IsRoundLineTaxAmt);
        }
        /** Get Round off Line Tax Amount.
        @return Round off Line Tax Amount */
        public Boolean IsRoundLineTaxAmt()
        {
            Object oo = Get_Value("IsRoundLineTaxAmt");
            if (oo != null)
            {
                if (oo.GetType() == typeof(bool))
                    return Convert.ToBoolean(oo);
