...
namespace VAdvantage.DataBase
{
    ...
    {/**	Logger	*/
        ...
        public static String UpdateSetSelectList(String sql)
        {
            ...
            // when Set List not matched with select list
            if (setList.Length != selectList.Length)
            {
                ...
                for (int i = 0; i < selectList.Length; i++)
                {
                    ...
                    startbracket += selectList[i].Count(f => (f == '('));
                    endbracket += selectList[i].Count(f => (f == ')'));
