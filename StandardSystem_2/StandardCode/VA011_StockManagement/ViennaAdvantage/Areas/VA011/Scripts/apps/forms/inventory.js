...
; (function (VA011, $) {
    ...
    VA011.InventoryForm = function () {
        ...
        function LoadWarehouses() {
            ...
            VIS.dataContext.getJSONData(VIS.Application.contextUrl + "Inventory/GetOrgWarehouseAll", { "value": "", "orgs": selOrgs, "fill": false }, LoadWarehouseCallBack);
        ...
        function ReLoadWarehouses() {
            ...
            //var qry = "SELECT M_Warehouse_ID, Name FROM M_Warehouse WHERE AD_Client_ID = " + VIS.context.getContext("#AD_Client_ID") + " AND IsActive = 'Y'";
            VIS.dataContext.getJSONData(VIS.Application.contextUrl + "Inventory/GetOrgWarehouseAll", { "value": "", "orgs": selOrgs, "fill": false }, ReLoadWarehouseCallBack);
            //VIS.DB.executeReader(qry, null, LoadWarehouseCallBack);
        };
        ...
        function createPanels() {
            ...
            $divRightSection = $('<div id="VA011_RightSection' + $self.windowNo + '" class="VA011-Rightpanel-DetailScroll"  >');
            ...
            $table = $("<table style='width:100%;height:100%;background-color:white;' class='vis-forms-container' >"); //main root
            $table.append($tr);
            $root.append($table);
        ...
        var LoadCart = function () {
            ...
            VIS.dataContext.getJSONData(VIS.Application.contextUrl + "Inventory/GetCart", "", LoadcartCallBack);
        ...
        function BindCartGrid() {
            ...
            if (!initLoad) {
                ...
                if (cmbCart.val() > 0) {
                    ...
                    VIS.dataContext.getJSONData(VIS.Application.contextUrl + "Inventory/LoadBindcart", { "VAICNT_InventoryCount_ID": cmbCart.val() }, BindCartCallBack);
                    ...
                    multiValues.push(
                        {
                            recid: Recid,
                            LineID: dr[i]["LineID"],
                            product_ID: dr[i]["product_ID"],
                            Product: dr[i]["Product"],
                            C_Uom_ID: VIS.Utility.Util.getValueOfInt(dr[i]["C_Uom_ID"]),
                            attribute_ID: VIS.Utility.Util.getValueOfInt(dr[i]["attribute_ID"]),
                    ...
                    grdSubstituteProdValues.push(
                        {
                            recid: Recid,
                            Product: dr[i]["Product"],
                            QtyOnHand: dr[i]["QtyOnHand"],
                            UOM: dr[i]["UOM"],
                            Reserved: dr[i]["Reserved"],
                            ATP: dr[i]["ATP"],
                            M_Product_ID: VIS.Utility.Util.getValueOfInt(dr[i]["M_Product_ID"]),
                        });
                }
            }
            //if (ds != null && ds.tables[0].rows.length > 0) {
        ...
        function gridSuppliersRightPanel() {
            ...
            dSuppliersRightGrid = $divSuppliersRightGrid.w2grid({
                ...
                columns: [
                    ...
                    { field: "DeliveryTime", caption: '<div><span>' + VIS.Msg.getMsg("VA011_DeliveryTime") + '</span></div>', sortable: false, size: '10%', hidden: false, render: 'number:2' },
                    { field: "M_Product_ID", caption: "M_Product_ID", sortable: false, size: '80px', display: false }
                ],
                records: [
        ...
        function bindSuppliersRightGrid() {
            ...
            if (!initLoad) {
                ...
                VIS.dataContext.getJSONData(VIS.Application.contextUrl + "Inventory/LoadSuppliersRightGrid", { "M_Product_ID": _product_ID }, callbackSuppliersRightGrid);
            ...
            if (dr != null && dr.length > 0) {
                for (var i = 0; i < dr.length; i++) {
                    ...
                    grdSuppliersRightProdValues.push(
                        {
                            recid: Recid,
                            Supplier: dr[i]["Supplier"],
                            QtyOrderPack: dr[i]["Qty"],
                            UOM: dr[i]["UOM"],
                            MinOrder: dr[i]["OrderMin"],
                            DeliveryTime: dr[i]["DeliveryTime"],
                            M_Product_ID: VIS.Utility.Util.getValueOfInt(dr[i]["product_ID"]),
                        });
                }
            }
            //if (ds != null && ds.tables[0].rows.length > 0) {
        ...
        function bindVariantGrid() {
            ...
            //sqlVar = "SELECT Name,  M_Product_ID,  UOM,  lot,  serno,  guaranteedate,  SUM(QtyOnHand) AS QtyOnHand,  M_AttributeSetInstance_ID,  Description FROM  ( ";
            //if (selWh.length > 0) {
            //    var whString = "";
            //    for (var w = 0; w < selWh.length; w++) {
            //        if (w == 0) {
            ...
            VIS.dataContext.getJSONData(VIS.Application.contextUrl + "Inventory/LoadVariantGrid", { "M_Product_ID": _product_ID, "selWH": selWh, "orgs": orgString }, callbackVariantGrid);

        };

        function callbackVariantGrid(dr) {
            ...
            if (dr != null && dr.length > 0) {
                for (var i = 0; i < dr.length; i++) {
                    ...
                    if (dr[i]["guaranteedate"] == null) {
                        ...
                        expDate = getDate(dr[i]["guaranteedate"]);
                    }
                    grdVariantProdValues.push(
                        {
                            ...
                            QtyOnHand: dr[i]["QtyOnHand"],
                            UOM: dr[i]["UOM"],
                            SerialNo: dr[i]["SerialNo"],
                            LotNo: dr[i]["LotNo"],
                            ExpDate: expDate,
                            M_Product_ID: dr[i]["M_Product_ID"],
                        });
                }
            }
            //var params = M_Product_ID.toString().concat(",", (mTab.getValue("description")).toString() +
            //    "," + Util.getValueOfString(mTab.getValue("qtyonhand")) +
            //    "," + Util.getValueOfString(mTab.getValue("uom")) +
            //    "," + Util.getValueOfString(mTab.getValue("serno")) +
            //    "," + Util.getValueOfString(mTab.getValue("lot")) +
            //    "," + Util.getValueOfString(mTab.getValue("expDate")) +
            ...
            //    for (var i = 0; i < ds.tables[0].rows.length; i++) {
            //        Recid = Recid + 1;
            //        if (ds.tables[0].rows[i].cells.guaranteedate == null) {
            //            expDate = null;
            //        }
            //        else {
        ...
        function callbackLocatorGrid(dr) {
            ...
            if (dr != null && dr.length > 0) {
                for (var i = 0; i < dr.length; i++) {
                    ...
                    if (dr[i]["LastReceipt"] == null) {
                        ...
                        lastReceipt = getDate(dr[i]["LastReceipt"]);
                    }
                    grdLocatorProdValues.push(
                        {
                            recid: Recid,
                            Warehouse: dr[i]["Warehouse"],
                            Locator: dr[i]["Locator"],
                            Quantity: dr[i]["Quantity"],
                            Unconfirmed: dr[i]["Unconfirmed"],
                            Attribute: dr[i]["Attribute"],
                            LastReceipt: dr[i]["LastReceipt"],
                            M_Product_ID: dr[i]["M_Product_ID"],
                        });
                }
            }
            ...
            //    for (var i = 0; i < ds.tables[0].rows.length; i++) {
            //        Recid = Recid + 1;
            //        if (ds.tables[0].rows[i].cells.lastreceipt == null) {
            //            lastReceipt = null;
            //        }
            //        else {
        ...
        function bindOrderedGrid() {
            ...
            if (!initLoad) {
                ...
                VIS.dataContext.getJSONData(VIS.Application.contextUrl + "Inventory/LoadOrderedGrid", { "M_Product_ID": _product_ID, "Warehouses": selWh, }, callbackOrderedGrid);

                //VIS.DB.executeDataSet(sqlOrd.toString(), null, callbackOrderedGrid);
            }
        };

        function callbackOrderedGrid(dr) {
            var Recid = 0;
            var datePromised = null;
            var dateOrdered = null;
            if (dr != null && dr.length > 0) {
                for (var i = 0; i < dr.length; i++) {
                    Recid = Recid + 1;


                    if ((dr[i]["DatePromised"]) == null) {
                        datePromised = null;
                    }
                    else {
                        datePromised = getDate(dr[i]["DatePromised"]);
                    }

                    if ((dr[i]["DateOrdered"]) == null) {
                        dateOrdered = null;
                    }
                    else {
                        dateOrdered = getDate(dr[i]["DateOrdered"]);
                    }
                    grdOrderedProdValues.push(
                        {
                            recid: Recid,
                            DatePromised: dr[i]["DatePromised"],
                            Quantity: dr[i]["Quantity"],
                            DateOrdered: dr[i]["DateOrdered"],
                            Supplier: dr[i]["Supplier"],
                            QtyReserved: dr[i]["QtyReserved"],
                            M_Product_ID: 0,
                        });
                }
            }
            ...
            //        if ((ds.tables[0].rows[i].cells.datepromised) == null) {
            //            datePromised = null;
            //        }
            //        else {
            ...
            //        if ((ds.tables[0].rows[i].cells.dateordered) == null) {
            //            dateOrdered = null;
            //        }
            //        else {
        ...
        function bindReplenishedGrid() {
            ...
            if (!initLoad) {
                ...
                //                Recid = Recid + 1;
                //                if (dsRep.tables[0].rows[i].cells.datedoc == null) {
                //                    date = null;
                //                }
                //                else {
        ...
        function callbackReplenishGrid(dr) {
            ...
            if (dr != null && dr.length > 0) {
                for (var i = 0; i < dr.length; i++) {
                    ...
                    if (dr[i]["datedoc"] == null) {
                        date = null;
                    }
                    else {
                        date = getDate(dr[i]["datedoc"]);
                    }

                    grdReplenishedProdValues.push(
                        {
                            recid: Recid,
                            RequisitionNo: dr[i]["RequisitionNo"],
                            Date: dr[i]["Date"],
                            QtyDemanded: dr[i]["QtyDemanded"],
                            QtyReceived: dr[i]["QtyReceived"],
                            QtyPending: dr[i]["QtyPending"],
                            // LastReceipt: dsRep.tables[0].rows[i].cells.lastreceipt,
                            M_Product_ID: dr[i]["M_Product_ID"],
                        });
                }
        ...
        function bindDemandGrid() {
            ...
            if (!initLoad) {
                ...
                VIS.dataContext.getJSONData(VIS.Application.contextUrl + "Inventory/LoadDemandGrid", { "M_Product_ID": _product_ID, "selWH": selWh, "orgs": orgString }, callbackDemandGrid);
        ...
        function bindTransactionsGrid() {
            ...
            if (!initLoad) {
                ...
                VIS.dataContext.getJSONData(VIS.Application.contextUrl + "Inventory/LoadTransactionsGrid", { "M_Product_ID": _product_ID, "selWH": selWh }, callbackBindTransactionsGrid);
        ...
        function callbackBindTransactionsGrid(dr) {
            ...
            //        if (ds.tables[0].rows[i].cells.movementdate == null) {
            ...
            dReplenishmentPopGrid = $divReplenishmentPopGrid.w2grid({
                ...
                columns: [
                    ...
                    //{ field: "SourceWarehouse", caption: VIS.Msg.getMsg("VA011_SourceWarehouse"), sortable: false, size: '20%' },
                    {
                        ...
                        render: function (record, index, col_index) {
                            var html = '';
                            for (var p in listSrcWarehouses) {
                                if (listSrcWarehouses[p].id == this.getCellValue(index, col_index)) html = listSrcWarehouses[p].text;
                            }
                            return html;
                        }
                    },
        ...
        function bindReplenishmentPopGrid() {
            ...
            if (!initLoad) {
                ...
                //callbackModuleCheckRepAll(sqlWhere);
                ...
                //    sqlRep += sqlWhere;
                //}
                VIS.dataContext.getJSONData(VIS.Application.contextUrl + "Inventory/LoadReplenishmentPopGrid", { "M_Warehouse_ID": $cmbRepAllWarehouse.val(), "sqlWhere": sqlWhere }, callbackBindReplenishmentPopGrid);

                //$bsyDiv[0].style.visibility = "hidden";
            }
        };

        function callbackModuleCheckRepAll(sqlWhere) {
            var sqlRep = "";
        ...
        function callbackBindReplenishmentPopGrid(dr) {
                    ...
                    grdReplenishmentPopValues.push(
                        {
                            ...
                            Max: dr[i]["Max"],
                            Qty: dr[i]["Qty"],
                            OrderPack: dr[i]["OrderPack"],
                            SourceWarehouse: dr[i]["SourceWarehouse"],
                            M_Warehouse_ID: dr[i]["M_Warehouse_ID"],
                            M_Product_ID: dr[i]["M_Product_ID"],
                        });
                }
            }
        ...
        function gridReplenishmentBPanel() {
            ...
            dReplenishmentBGrid = $divReplenishmentBGrid.w2grid({
                ...
                columns: [
                    ...
                    //{
                    //    field: "SourceWarehouse", caption: '<div style="text-align: center;" ><span>' + VIS.Msg.getMsg("VA011_SourceWarehouse") + '</span></div>', sortable: false, size: '20%', hidden: false,
                    //    render: function (record, index, col_index) {
                    //        var html = '<div><select style="width:100%;"></select></div>';
                    //        for (var p in listSrcWarehouses) {
                    //            if (listSrcWarehouses[p].id == this.getCellValue(index, col_index))
                    //                html = '<div><select style="width:100%;"><option value=' + listSrcWarehouses[p].id + '>' + VIS.Utility.encodeText(listSrcWarehouses[p].text) + '</option></select></div>';
                    //        }
                    //        return html;
                    //    }
                    //},
        ...
        function bindReplenishmentBGrid() {
            ...
            if (!initLoad) {
                ...
                VIS.dataContext.getJSONData(VIS.Application.contextUrl + "Inventory/LoadReplenishmentBGrid", { "M_Product_ID": _product_ID, "selWH": selWh }, callbackBindReplenishmentBGrid);
        ...
        function gridProductPanel() {
            ...
            dProdGrid = $divProdGrid.w2grid({
                ...
                columns: [
                    ...
                    {
                        field: "Status", caption: '<div style="text-align: center;" ><span>' + VIS.Msg.getMsg("VA011_Status") + '</span></div>', sortable: false, size: '80px', hidden: false,
                        render: function () {
        ...
        function callbackBindProdReplenish(res) {
            ...
            for (var i = 0; i < res.length; i++) {
                if (res[i] == -999999999) {
                    dProdGrid.records[i]["QtyToReplenish"] = 0;
                    lstNoRepRule.push(i);
                }
                else {
            ...
            if (calculateReplenishSelected) {
                if (dProdGrid.records != null) {
                    for (var i = 0; i < dProdGrid.records.length; i++) {
                        ...
                        if (lstNoRepRule.map(function (item) { return item == i; }).indexOf(true) >= 0) {
        ...
        function LoadDocTypeCombo() {
            ...
            VIS.dataContext.getJSONData(VIS.Application.contextUrl + "Inventory/GetDocType", "", callbackLoadCmbDocType);
        ...
        function LoadDocStatusCombo() {
            ...
            VIS.dataContext.getJSONData(VIS.Application.contextUrl + "Inventory/GetDocStatus", "", callbackLoadDocStatus);
        ...
        function LoadReplenishType() {
            ...
            VIS.dataContext.getJSONData(VIS.Application.contextUrl + "Inventory/GetReplenishType", "", callbackReplenishType);
        ...
        function callbackReplenishType(dr) {
            ...
            if (dr != null && dr.length > 0) {
                for (var i = 0; i < dr.length; i++) {
                    Name = dr[i].Name;
                    Key = dr[i].Key;
                    repTypeArray.push({ id: key, text: Name });
                }
            }
            //while (dr.read()) {
            //    key = (dr.getString(0));
        ...
        function LoadCreateCombo() {
            ...
            VIS.dataContext.getJSONData(VIS.Application.contextUrl + "Inventory/CreateCombo", "", callbackLoadCmbCreate);
        ...
        function LoadUOM() {
            ...
            VIS.dataContext.getJSONData(VIS.Application.contextUrl + "Inventory/GetUOM", "", LoadUOMCallBack);
        ...
        function calculateReplenish() {
            if (dProdGrid.records.length > 0) {
                ...
                $bsyDiv[0].style.visibility = "visible";
                VIS.dataContext.getJSONData(VIS.Application.contextUrl + "Inventory/GetReplenish", { "ColumnName": prods, "whIDs": selWh }, callbackBindProdReplenish);
            }
        };
        ...
        function events() {
            ...
            $divProductCategories.on("click touchstart", function (e) {
                e.preventDefault();
                e.stopPropagation();
            ...
            $divOrg.on("click touchstart", function (e) {
                e.preventDefault();
                e.stopPropagation();
                if (e.target.className.indexOf("VA011-imgCombo") >= 0) {
            ...
            $divWarehouse.on("click touchstart", function (e) {
                e.preventDefault();
                e.stopPropagation();
                if (e.target.className.indexOf("VA011-imgCombo") >= 0) {
            ...
            $divPLV.on("click touchstart", function (e) {
                e.preventDefault();
                e.stopPropagation();
                if (e.target.className.indexOf("VA011-imgCombo") >= 0) {
            ...
            $divSupplier.on("click touchstart", function (e) {
                e.preventDefault();
                e.stopPropagation();
                if (e.target.className.indexOf("VA011-imgCombo") >= 0) {
            ...
            $inputSearchBtnProd.on("click touchstart", function (e) {
                e.preventDefault();
                e.stopPropagation();
            ...
            $divHeadDetails.on("click touchstart", "LI", function (e) {
                e.preventDefault();
                e.stopPropagation();
            ...
            $rightFullDetails.on("click touchstart", "LI", function (e) {
                e.preventDefault();
                e.stopPropagation();
                var btn = $(e.target).attr("id");
                //if (btn.indexOf("VA011_btnDetails_") >= 0) {
            ...
            $divHeadVarientProdTop.on("click touchstart", "LI", function (e) {
                e.preventDefault();
                e.stopPropagation();
                var btn = $(e.target).attr("id");
            ...
            $divHeadVarientProdBottom.on("click touchstart", "LI", function (e) {
                e.preventDefault();
                e.stopPropagation();
                if (!multipleRecSelected) {
                    var btn = $(e.target).attr("id");
            ...
            if ($ulLefttoolbar) {
                $ulLefttoolbar.on("click touchstart", "LI", function (e) {
                    e.preventDefault();
                    e.stopPropagation();
            ...
            $ulStatusProdDiv.on("click touchstart", "LI", function (e) {
                e.preventDefault();
                e.stopPropagation();
            ...
            $btnlbToggle.on("click touchstart", function (e) {
                e.preventDefault();
                e.stopPropagation();
            ...
            $divCheckBoxOrg.on("click touchstart", function (ev) {
                ev.preventDefault();
                ev.stopPropagation();
                var tgt = ev.target;
            ...
            $divCheckBoxPLV.on("click touchstart", function (ev) {
                ev.preventDefault();
                ev.stopPropagation();
                var tgt = ev.target;
                if (tgt.className != "VA011-clsDIV") {
            ...
            $divCheckBoxSupplier.on("click touchstart", function (ev) {
                ev.preventDefault();
                ev.stopPropagation();
            ...
            $divCheckBoxWarehouse.on("click touchstart", function (ev) {
                ev.preventDefault();
                ev.stopPropagation();
                var tgt = ev.target;
                if (tgt.className != "VA011-clsDIV") {
            ...
            $divCheckBoxCategories.on("click touchstart", function (ev) {
                ev.preventDefault();
                ev.stopPropagation();
                var tgt = ev.target;
                if (tgt.className != "VA011-clsDIV") {
            ...
            btnNewCart.on("click touchstart", function (e) {
                e.preventDefault();
                e.stopPropagation();
                divCartList.hide();
                divNewCart.show();
            ...
            btnEditCart.on("click touchstart", function (e) {
                e.preventDefault();
                e.stopPropagation();
                zoomToWindow(cmbCart.val(), "VAICNT_InventoryCount");
            });

            btnRefreshCart.on("click touchstart", function (e) {
                e.preventDefault();
                e.stopPropagation();
                LoadCart();
            });

            btnSaveScan.on("click touchstart", function (e) {
                e.preventDefault();
                e.stopPropagation();
            ...
            btnCancelScan.on("click touchstart", function (e) {
                e.preventDefault();
                e.stopPropagation();
                divNewCart.hide();
                divCartList.show();
